...
function applyTugRoll(state: TugState, roll: number): TugState {
  ...
  const distanceToFinish = Math.abs(targetFinish - state.position);
  const playerName = state.currentPlayer === "p1" ? state.player1Name : state.player2Name;
  const nextPlayer = state.currentPlayer === "p1" ? "p2" : "p1";

  if (roll > distanceToFinish) {
    return {
      ...state,
      lastRoll: roll,
      currentPlayer: nextPlayer,
      rollCount: state.rollCount + 1,
      message: `${playerName} rolled ${roll}, needs exact ${distanceToFinish}`,
    };
  }

  const nextPosition = state.position + direction * roll;
...
export function DiceTugGame({ onBack }: { onBack: () => void }) {
  ...
  const exactNeed = Math.abs(activeDirection * finishDistance - state.position);
...
function TugCell({ cell }: { cell: number }) {
  ...
  return (
    <group position={[cell * 0.82, 0.22, 0]}>
      ...
      <Text
        ...
        outlineWidth={0.018}
      >
        {isStart ? "START" : isFinish ? "FINISH" : Math.abs(cell)}
      </Text>
    </group>
  );
}
